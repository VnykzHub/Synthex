...
- Installing Python dependencies via pip before running the prototype.
...
6. **Install dependencies first.** Before running the prototype, install required packages via pip. Generate `requirements.txt` with pinned versions and run `pip install -r requirements.txt` before the first launch.
...
6. **Wire persistence.** Use SQLite (via `sqlite3` or SQLAlchemy) for any data that should survive restarts. Keep the DB file in `agent-output/artifacts/{prototype-name}/` and include a `schema.sql` if the schema is non-trivial. Use the prototype name in the DB filename (e.g., `{prototype-name}.db`) to avoid collisions when multiple prototypes coexist.
...
8. **Log the intent.** Call `log_intent(agent="prototyping", action="prototype.created", why="<rationale>", context="<prototype-name>")` to persist the decision for audit trail continuity.
...
- Database files (if any) go to `agent-output/artifacts/{prototype-name}/`. Include a `schema.sql` if the schema is non-trivial.
